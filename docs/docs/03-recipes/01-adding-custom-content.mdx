...
<CodeBlock language="yaml" showLineNumbers={true}>{`url: https://example.com/my-file.tar.gz`}</CodeBlock>
...
<CodeBlock language="yaml" showLineNumbers={true}>{`urlFrom:
  mavenRef:
...
<CodeBlock language="yaml" title="server.yaml" showLineNumbers={true}>{`apiVersion: shulkermc.io/v1alpha1
